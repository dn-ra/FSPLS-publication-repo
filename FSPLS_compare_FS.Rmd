...
saveRDS(auc_results, file = 'output/golub_forward.stagewise_aucs_10_folds.Rds')
...
saveRDS(auc_results, file = 'output/kaf_forward.stagewise_aucs.Rds')
```
...
lars.mod <- lars(as.matrix(trainx_normalised), as.matrix(trainy), use.Gram = F, normalize = F, type = 'forward.stagewise', max.steps = 6)
new_preds <- predict(lars.mod, testx_normalised)

aucs[[i]] <- pROC::auc(response = testy, predictor = as.numeric(new_preds$fit[,5]))

}

saveRDS(aucs, file = 'Ng_forward.stagewise_aucs.Rds')
